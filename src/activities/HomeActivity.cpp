...
// |   1.6. constructors     |
// +++++++++++++++++++++++++++
HomeActivity::HomeActivity() : XiaoEActivity(&HomeActivity::window_template){
    m_style = STYLE_PUSH;
...
BOOL HomeActivity::onEraseBackground(mMainWnd *self, HDC hdc, const PRECT clip)
{
    RECT rc;
    GetClientRect(self->hwnd, &rc);
...
BOOL HomeActivity::onScreensave(mMainWnd* self, int message, WPARAM wParam, LPARAM lParam)
{
...
NCS_MNWND_TEMPLATE HomeActivity::window_template = 
    XIAOE_WINDOW_TEMPLATE(ACTIVITY_NAME, HomeActivity::control_templates);
